...
public class DialogueDebugNavigator : MonoBehaviour
{
    ...
    // Rate-limit common warnings (prevents log spam from UI buttons)
    private float lastNoBackWarningTime = -999f;
    private const float NoBackWarningCooldownSeconds = 2f;
    ...
    private JumpMenuItem[] jumpMenuItems = new JumpMenuItem[]
    {
        ...
        new JumpMenuItem { displayName = "Run 2 Day 1", nodeName = "R2_D1_ArthurOrientation" },
    ...
    private void Awake()
    {
        // NOTE: Intentionally allowed in non-development builds.
        // This project uses this component for player-visible controls as well.

        // Never show the small yellow debug overlay in non-debug builds, even if enabled in the inspector.
#if !UNITY_EDITOR
    ...
    private void Update()
    {
        ...
        // Don't allow debug navigation while a modal UI (like feedback form) is open.
    ...
    /// <summary>
    /// Opens the Jump-to-Day menu (same menu as Ctrl+J).
    /// </summary>
    ...
    private void JumpToDay(string nodeName, string displayName)
    {
        ...
        // Validate the target node exists (prevents confusing errors if node names change)
        if (string.IsNullOrEmpty(nodeName) || !NodeExistsInProject(dialogueRunner.YarnProject, nodeName))
        {
            Debug.LogError($"[Debug Nav] Jump target node not found: '{nodeName}'. Update DialogueDebugNavigator.jumpMenuItems to match Yarn node titles.");
            return;
        }
    ...
    /// <summary>
    /// Checks if a node exists in a YarnProject without directly referencing protobuf MapField types.
    /// </summary>
    private static bool NodeExistsInProject(YarnProject project, string nodeName)
    {
        ...
        // YarnProject.NodeNames is a string[] and keeps protobuf details inside the Yarn Spinner assembly.
        string[] nodeNames = project.NodeNames;
        if (nodeNames == null)
        {
            return false;
        }
    ...
    public void GoBack()
    {
        ...
        {
            if (Time.unscaledTime - lastNoBackWarningTime >= NoBackWarningCooldownSeconds)
            {
                Debug.LogWarning("[Debug Nav] Cannot go back: No previous nodes in history.");
                lastNoBackWarningTime = Time.unscaledTime;
            }
            return;
        }
    ...
    private void OnGUI()
    {
        ...
        // Draw debug overlay
        if (showDebugOverlay && (Debug.isDebugBuild || Application.isEditor))
        {
            DrawDebugOverlay();
        }
    }
    ...
    private void DrawJumpMenu()
    {
        ...
        GUI.Label(new Rect(Screen.width / 2 - 200, Screen.height / 2 - 110, 400, 20), "Press 1-9 (0 for 10) to select, Escape to close", instructionStyle);
        ...
        for (int i = 0; i < jumpMenuItems.Length; i++)
        {
            ...
            string itemText = $"{keyLabel}. {jumpMenuItems[i].displayName}";
