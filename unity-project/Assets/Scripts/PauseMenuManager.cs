...
public class PauseMenuManager : MonoBehaviour
{
    ...
    // Always-visible runtime buttons (created programmatically)
    private Button bottomRightBackButton;
    private CanvasGroup bottomRightBackButtonCanvasGroup;
    private Button topLeftMenuButton;
    private CanvasGroup topLeftMenuButtonCanvasGroup;
    private Button bottomLeftFeedbackButton;
    private CanvasGroup bottomLeftFeedbackButtonCanvasGroup;
    private float cachedTopLeftMenuAlpha = 1f;
    private bool cachedTopLeftMenuAlphaValid = false;
    private float cachedBottomRightBackAlpha = 1f;
    private bool cachedBottomRightBackAlphaValid = false;
    private float cachedBottomLeftFeedbackAlpha = 1f;
    private bool cachedBottomLeftFeedbackAlphaValid = false;
    private DialogueDebugNavigator cachedDebugNavigator;

    // Keep Run/Day tracker in sync with dialogue node transitions (jump/back).
    private DialogueRunner hookedDialogueRunner;
    private bool isHookedToNodeStart = false;
    private Coroutine pendingRunDayRefresh;
    ...
    {
        // Prefer the same Canvas that the pause menu lives under (ensures visibility on WebGL builds).
        ...
        // Fallback to deterministic overlay canvas (used elsewhere for runtime UI).
        Canvas overlayCanvas = OverlayCanvasProvider.GetCanvas();
    ...
    private void Start()
    {
        ...
        SetupRunDayTracker();
        EnsureDialogueRunnerNodeStartHook();
        ...
        if (exitButton != null)
        {
            // Repurpose exit button as Debug Menu for now (opens Ctrl+J jump menu)
            SetButtonLabel(exitButton, "Debug Menu");
            exitButton.onClick.RemoveAllListeners();
            exitButton.onClick.AddListener(OnDebugMenu);
        }
        else
        {
    ...
    private void Update()
    {
        // If a modal UI is open, do not allow pause/menu/back interactions from this manager.
        // (Modal itself will handle Escape.)
        UpdateModalUiVisibility();

        // DialogueRunner may appear after scene load; keep hook alive.
        EnsureDialogueRunnerNodeStartHook();

        // Check for ESC key to toggle pause (using new Input System)
        if (!ModalInputLock.IsLocked &&
            !ModalInputLock.WasEscapeConsumedThisFrame &&
        ...
        // Keep Back button availability in sync with dialogue history
        if (!ModalInputLock.IsLocked)
        {
    ...
    private void UpdateModalUiVisibility()
    {
        bool locked = ModalInputLock.IsLocked;

        // Hide/disable the always-visible Menu and Back buttons while locked.
        if (topLeftMenuButton != null)
        {
            if (topLeftMenuButtonCanvasGroup == null)
            {
                topLeftMenuButtonCanvasGroup = topLeftMenuButton.GetComponent<CanvasGroup>();
                if (topLeftMenuButtonCanvasGroup == null)
                {
                    topLeftMenuButtonCanvasGroup = topLeftMenuButton.gameObject.AddComponent<CanvasGroup>();
                }
            }

            if (locked)
            {
                if (!cachedTopLeftMenuAlphaValid)
                {
                    cachedTopLeftMenuAlpha = topLeftMenuButtonCanvasGroup.alpha;
                    cachedTopLeftMenuAlphaValid = true;
                }

                topLeftMenuButton.interactable = false;
                topLeftMenuButtonCanvasGroup.alpha = 0f;
                topLeftMenuButtonCanvasGroup.blocksRaycasts = false;
            }
            else
            {
                topLeftMenuButton.interactable = true;
                topLeftMenuButtonCanvasGroup.blocksRaycasts = true;
                if (cachedTopLeftMenuAlphaValid)
                {
                    topLeftMenuButtonCanvasGroup.alpha = cachedTopLeftMenuAlpha;
                    cachedTopLeftMenuAlphaValid = false;
                }
                else
                {
                    topLeftMenuButtonCanvasGroup.alpha = 1f;
                }
            }
        }

        if (bottomRightBackButton != null)
        {
            if (bottomRightBackButtonCanvasGroup == null)
            {
                bottomRightBackButtonCanvasGroup = bottomRightBackButton.GetComponent<CanvasGroup>();
                if (bottomRightBackButtonCanvasGroup == null)
                {
                    bottomRightBackButtonCanvasGroup = bottomRightBackButton.gameObject.AddComponent<CanvasGroup>();
                }
            }

            if (locked)
            {
                if (!cachedBottomRightBackAlphaValid)
                {
                    cachedBottomRightBackAlpha = bottomRightBackButtonCanvasGroup.alpha;
                    cachedBottomRightBackAlphaValid = true;
                }

                bottomRightBackButton.interactable = false;
                bottomRightBackButtonCanvasGroup.alpha = 0f;
                bottomRightBackButtonCanvasGroup.blocksRaycasts = false;
            }
            else
            {
                bottomRightBackButtonCanvasGroup.blocksRaycasts = true;
                if (cachedBottomRightBackAlphaValid)
                {
                    bottomRightBackButtonCanvasGroup.alpha = cachedBottomRightBackAlpha;
                    cachedBottomRightBackAlphaValid = false;
                }
                // Note: interactable is restored by UpdateBackButtonState().
            }
        }

        if (bottomLeftFeedbackButton != null)
        {
            if (bottomLeftFeedbackButtonCanvasGroup == null)
            {
                bottomLeftFeedbackButtonCanvasGroup = bottomLeftFeedbackButton.GetComponent<CanvasGroup>();
                if (bottomLeftFeedbackButtonCanvasGroup == null)
                {
                    bottomLeftFeedbackButtonCanvasGroup = bottomLeftFeedbackButton.gameObject.AddComponent<CanvasGroup>();
                }
            }

            if (locked)
            {
                if (!cachedBottomLeftFeedbackAlphaValid)
                {
                    cachedBottomLeftFeedbackAlpha = bottomLeftFeedbackButtonCanvasGroup.alpha;
                    cachedBottomLeftFeedbackAlphaValid = true;
                }

                bottomLeftFeedbackButton.interactable = false;
                bottomLeftFeedbackButtonCanvasGroup.alpha = 0f;
                bottomLeftFeedbackButtonCanvasGroup.blocksRaycasts = false;
            }
            else
            {
                bottomLeftFeedbackButton.interactable = true;
                bottomLeftFeedbackButtonCanvasGroup.blocksRaycasts = true;
                if (cachedBottomLeftFeedbackAlphaValid)
                {
                    bottomLeftFeedbackButtonCanvasGroup.alpha = cachedBottomLeftFeedbackAlpha;
                    cachedBottomLeftFeedbackAlphaValid = false;
                }
                else
                {
                    bottomLeftFeedbackButtonCanvasGroup.alpha = 1f;
                }
            }
        }
    }
    ...
    {
        UnhookDialogueRunnerNodeStart();
    }

    private void EnsureDialogueRunnerNodeStartHook()
    {
        if (dialogueRunner == null)
        {
            // Try to find if not assigned yet.
            dialogueRunner = FindFirstObjectByType<DialogueRunner>();
        }
        ...
        // If the runner changed (scene reload), re-hook.
        if (hookedDialogueRunner != null && hookedDialogueRunner != dialogueRunner)
        {
            UnhookDialogueRunnerNodeStart();
        }

        if (isHookedToNodeStart && hookedDialogueRunner == dialogueRunner)
        {
            return;
        }

        hookedDialogueRunner = dialogueRunner;
        hookedDialogueRunner.onNodeStart.AddListener(OnDialogueNodeStart);
        isHookedToNodeStart = true;
    }

    private void UnhookDialogueRunnerNodeStart()
    {
        if (pendingRunDayRefresh != null)
        {
            StopCoroutine(pendingRunDayRefresh);
            pendingRunDayRefresh = null;
        }

        if (hookedDialogueRunner != null && isHookedToNodeStart)
        {
            hookedDialogueRunner.onNodeStart.RemoveListener(OnDialogueNodeStart);
        }

        hookedDialogueRunner = null;
        isHookedToNodeStart = false;
    }

    private void OnDialogueNodeStart(string nodeName)
    {
        // Refresh immediately and again next frame to catch variables set during node start.
        UpdateRunDayTracker(nodeName);

        if (pendingRunDayRefresh != null)
        {
            StopCoroutine(pendingRunDayRefresh);
        }
        pendingRunDayRefresh = StartCoroutine(RefreshRunDayNextFrame());
    }

    private IEnumerator RefreshRunDayNextFrame()
    {
        yield return null;
        UpdateRunDayTracker();
        pendingRunDayRefresh = null;
    ...
    private void SetupRunDayTracker()
    {
        ...
        // If tracker text is not assigned, create it programmatically (boxed panel matching HUD buttons)
        if (runDayTrackerText == null)
        {
            // Avoid duplicates if scene already has one.
            Transform existing = canvas.transform.Find("RunDayTrackerPanel");
            if (existing != null)
            {
#if USE_TMP
                var existingTmp = existing.GetComponentInChildren<TMPro.TextMeshProUGUI>(true);
                if (existingTmp != null)
                {
                    runDayTrackerText = existingTmp;
                    return;
                }
#endif
                var existingText = existing.GetComponentInChildren<Text>(true);
                if (existingText != null)
                {
                    runDayTrackerText = existingText;
                    return;
                }
            }

            GameObject panelObj = new GameObject("RunDayTrackerPanel");
            ...
            panelRect.anchoredPosition = new Vector2(HudEdgePadding, -(HudEdgePadding + HudButtonHeight + HudVerticalGap));
            panelRect.sizeDelta = new Vector2(HudRunDayBoxWidth, HudButtonHeight);

            Image bg = panelObj.AddComponent<Image>();
            bg.color = HudBackgroundColor;
            bg.raycastTarget = false;

            GameObject trackerObj = new GameObject("Text");
            trackerObj.transform.SetParent(panelObj.transform, false);
    ...
    /// </summary>
    private void UpdateRunDayTracker(string nodeNameHint = null)
    {
        if (runDayTrackerText == null || dialogueRunner == null || dialogueRunner.VariableStorage == null) return;

        // Prefer parsing run/day from the current node name so it updates correctly when jumping/backing.
        string nodeName = nodeNameHint;
        if (string.IsNullOrEmpty(nodeName) && dialogueRunner.Dialogue != null)
        {
            nodeName = dialogueRunner.Dialogue.CurrentNode;
        }

        if (TryParseRunDayFromNode(nodeName, out int runFromNode, out int dayFromNode))
        {
            SetRunDayTrackerText(runFromNode, dayFromNode);
            return;
        }
        ...
        SetRunDayTrackerText(Mathf.RoundToInt(currentRun), Mathf.RoundToInt(currentDay));
    }

    private static bool TryParseRunDayFromNode(string nodeName, out int run, out int day)
    {
        run = 1;
        day = 1;
        if (string.IsNullOrEmpty(nodeName)) return false;

        Match m = RunDayNodeRegex.Match(nodeName);
        if (!m.Success) return false;

        if (!int.TryParse(m.Groups["run"].Value, out run)) return false;
        if (!int.TryParse(m.Groups["day"].Value, out day)) return false;
        return true;
    }

    private void SetRunDayTrackerText(int run, int day)
    {
        string trackerText = $"Run {run}, Day {day}";
    ...
    /// <summary>
    /// Debug Menu button handler - opens the dialogue jump menu (same as Ctrl+J).
    /// </summary>
    public void OnDebugMenu()
    {
        DialogueDebugNavigator nav = FindFirstObjectByType<DialogueDebugNavigator>();
        if (nav == null)
        {
            Debug.LogWarning("PauseMenuManager: Debug menu requested but DialogueDebugNavigator was not found in scene.");
            return;
        }
    ...
    {
        if (bottomRightBackButton == null) return;

        if (cachedDebugNavigator == null)
        {
            cachedDebugNavigator = FindFirstObjectByType<DialogueDebugNavigator>();
        }

        bool canGoBack = cachedDebugNavigator != null && cachedDebugNavigator.GetHistoryCount() > 1;

        bottomRightBackButton.interactable = canGoBack;
        if (bottomRightBackButtonCanvasGroup != null)
        {
            bottomRightBackButtonCanvasGroup.alpha = canGoBack ? 1f : 0.35f;
        }
    }

    /// <summary>
    /// Sets the visible label on a Unity UI Button (supports Text or TMP text child).
    /// </summary>
        ...
        UnityEngine.Events.UnityAction onClick)
    {
        ...
        {
            return canvas.transform.Find(name).GetComponent<Button>();
        }
        ...
        image.color = HudBackgroundColor;
    ...
    private void SetupTopLeftMenuButton()
    {
        Canvas canvas = GetUiCanvas();
        if (canvas == null) return;

        topLeftMenuButton = CreateHudButton(
            canvas,
            "TopLeftMenuButton",
            new Vector2(0f, 1f),
            new Vector2(0f, 1f),
            ...
            new Vector2(HudMenuButtonWidth, HudButtonHeight),
            "Menu",
            () =>
            {
                if (ModalInputLock.IsLocked)
    ...
    private void SetupBottomLeftFeedbackButton()
    {
        Canvas canvas = GetUiCanvas();
        if (canvas == null) return;

        bottomLeftFeedbackButton = CreateHudButton(
            canvas,
            "BottomLeftFeedbackButton",
            new Vector2(0f, 0f),
            new Vector2(0f, 0f),
            new Vector2(0f, 0f),
            new Vector2(HudEdgePadding, HudEdgePadding),
            new Vector2(HudFeedbackButtonWidth, HudButtonHeight),
            "Feedback",
            () =>
            {
                if (ModalInputLock.IsLocked)
                ...
                FeedbackForm form = FindFirstObjectByType<FeedbackForm>();
                if (form == null)
                {
                    Debug.LogWarning("PauseMenuManager: Feedback requested but FeedbackForm was not found in scene.");
                    return;
                }
    ...
    private void SetupBottomRightBackButton()
    {
        Canvas canvas = GetUiCanvas();
        if (canvas == null) return;

        bottomRightBackButton = CreateHudButton(
            canvas,
            ...
            () =>
            {
                ...
                DialogueDebugNavigator nav = cachedDebugNavigator != null ? cachedDebugNavigator : FindFirstObjectByType<DialogueDebugNavigator>();
                cachedDebugNavigator = nav;
                if (nav == null)
                {
                    Debug.LogWarning("PauseMenuManager: Back requested but DialogueDebugNavigator was not found in scene.");
                    return;
                }
                nav.GoBack();
            });
        ...
        bottomRightBackButton.interactable = false; // start disabled until we have history
        ...
        bottomRightBackButtonCanvasGroup.alpha = 0.35f;
    }

    private static void CreateButtonText(Transform buttonTransform, string textValue)
    {
        ...
        // Padding from left edge of the button.
    ...
    {
        // Clean up node-start hook (prevents duplicate listeners across scene reloads)
        UnhookDialogueRunnerNodeStart();
