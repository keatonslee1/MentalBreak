...
public class AudioCommandHandler : MonoBehaviour
{
    ...
    public void PlayBGM(string key)
    {
        ...
        if (clip != null)
        {
            if (bgmSource != null)
            {
                // If the requested clip is already the current clip, don't restart it.
                // This prevents repeated <<bgm ...>> commands across nodes from resetting the music.
                if (bgmSource.clip == clip)
                {
                    ...
                    // If it's paused, UnPause continues from the current time; otherwise Play starts it.
                    if (bgmSource.time > 0f)
                    {
                        bgmSource.UnPause();
                        Debug.Log($"BGM: Resuming {key} (clip: {clip.name})");
                    }
                    else
                    {
                        bgmSource.Play();
