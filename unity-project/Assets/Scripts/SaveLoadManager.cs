...
public class SaveLoadManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Initialize saves directory (non-WebGL). WebGL uses IndexedDB-backed PlayerPrefs instead.
#if !(UNITY_WEBGL && !UNITY_EDITOR)
...
#if UNITY_WEBGL && !UNITY_EDITOR
            // WebGL: Persist client-side via PlayerPrefs (IndexedDB-backed).
            string key = GetWebGLSlotKey(slot);
            PlayerPrefs.SetString(key, json);
            PlayerPrefs.SetInt(WebGLLastSaveSlotKey, slot);
            PlayerPrefs.Save();

            Debug.Log($"Game saved (WebGL) to {slotType} slot {slot} at node {currentNodeName} (PlayerPrefs key '{key}')");
#else
    ...
    public bool LoadGame(int slot = 0)
    {
        ...
        try
        {
            ...
            {
                Debug.LogWarning($"SaveLoadManager: No WebGL save found for slot {slot} (missing PlayerPrefs key '{key}')");
                return false;
            }
            ...
            {
                Debug.LogWarning($"SaveLoadManager: WebGL save key '{key}' is empty.");
                return false;
            }
#else
            string filePath = GetSaveFilePath(slot);
    ...
    public SaveSlotData GetSaveSlotData(int slot)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(json))
            {
                Debug.LogError($"SaveLoadManager: Save data is empty for slot {slot}.");
                return null;
            }
            ...
            {
#if UNITY_WEBGL && !UNITY_EDITOR
                Debug.LogError($"SaveLoadManager: Failed to deserialize JSON from WebGL slot {slot} (PlayerPrefs key '{key}')");
#else
            ...
            {
#if UNITY_WEBGL && !UNITY_EDITOR
                Debug.LogError($"SaveLoadManager: SaveData is incomplete in WebGL slot {slot} (PlayerPrefs key '{key}')");
#else
        ...
        {
            Debug.LogError($"SaveLoadManager: Error reading save slot data for slot {slot}: {e.Message}\n{e.StackTrace}");
            return null;
        }
    }
    ...
    public bool DeleteSave(int slot)
    {
        ...
        try
        {
            ...
            PlayerPrefs.Save();
            Debug.Log($"Save slot {slot} deleted (WebGL PlayerPrefs key '{key}')");
            return true;
#else
            string filePath = GetSaveFilePath(slot);
            if (File.Exists(filePath))
